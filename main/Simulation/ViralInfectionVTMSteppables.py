...
def simple_drug_internalization_pk_string(met_rate, elimin_rate, time_of_first_dose, dosing_interval):
    pk_string = f"""

    model simple_pk()
    // times in MCS!
    J0: prodrug -> drug ; met_rate * prodrug
    J1: drug -> ; elimin_rate * drug

    // rates
    met_rate = {met_rate}
    elimin_rate = {elimin_rate}
    ...
    time_of_first_dose = {time_of_first_dose}

    dosing_interval = {dosing_interval} 

    // events

    E1: at (time - time_of_first_dose > 0): prodrug = 1
    E2: at ( (time-time_of_first_dose > dosing_interval) && sin((((time-time_of_first_dose)/dosing_interval))*2*pi)>0): prodrug=prodrug + 1 
    end
    """
...
class InternalizationBlockerSteppable(ViralInfectionVTMSteppableBasePy):
    ...
    def __init__(self, frequency=1):
        ...
        self.shared_steppable_vars['intern_ec50'] = .5

        self.set_intern_blocker_drug = simple_drug_internalization_pk_string
        self.intern_block_string = None
        self.blocker_rr = None

        self.met_rate = DEInputs.intern_met_rate
        self.elimin_rate = DEInputs.intern_elimin_rate
        self.time_of_first_dose = DEInputs.intern_time_1
        self.dosing_interval = DEInputs.intern_dosing_period

    def start(self):
        self.intern_block_string = self.set_intern_blocker_drug(self.met_rate, self.elimin_rate,
                                                                self.time_of_first_dose, self.dosing_interval)

        self.add_free_floating_antimony(model_string=self.intern_block_string, step_size=1,
                                        model_name='viral_internal_blocker_model')
        self.blocker_rr = get_roadrunner_for_single_antimony('viral_internal_blocker_model')
        self.shared_steppable_vars['intern_blocker_active'] = False
        self.base_rate = kon
        self.shared_steppable_vars['intern_ec50'] = .5
        self.shared_steppable_vars['intern_met'] = self.met_rate
        self.shared_steppable_vars['intern_elim'] = self.elimin_rate
        self.shared_steppable_vars['intern_dosing_interval'] = self.dosing_interval
        self.shared_steppable_vars['changed'] = False

    def step(self, mcs):
        self.blocker_rr.timestep()

        if self.shared_steppable_vars['changed']:
            self.sbml.viral_internal_blocker_model['met_rate'] = self.shared_steppable_vars['intern_met']
            self.sbml.viral_internal_blocker_model['elimin_rate'] = self.shared_steppable_vars['intern_elim']
            self.sbml.viral_internal_blocker_model['dosing_interval'] = \
                self.shared_steppable_vars['intern_dosing_interval']

        blocker_activated = self.shared_steppable_vars['intern_blocker_active']
        if blocker_activated:
            drug_amount = self.get_drug_amount()
            calculated_kon = self.calculate_rate(drug_amount)
            print(drug_amount, calculated_kon)
            # ViralInternalizationSteppable._internalization_rate = calculated_rate
            self.shared_steppable_vars['k_on'] = calculated_kon
        else:
            self.shared_steppable_vars['k_on'] = self.base_rate

    def calculate_rate(self, drug_amount):
        blocker_ec50 = self.shared_steppable_vars['intern_ec50']
        return (1 - nCoVUtils.hill_equation(drug_amount, blocker_ec50, 1)) * self.base_rate

    def get_drug_amount(self):
        return self.sbml.viral_internal_blocker_model['drug']


class ReplicationBlockerSteppable(ViralInfectionVTMSteppableBasePy):
    def __init__(self, frequency=1):
        ViralInfectionVTMSteppableBasePy.__init__(self, frequency)

        self.base_rate = replicating_rate

        self.shared_steppable_vars['rep_ec50'] = .5

        self.set_rep_blocker_drug = simple_drug_internalization_pk_string
        self.rep_block_string = None
        self.blocker_rr = None

        self.met_rate = DEInputs.rep_met_rate
        self.elimin_rate = DEInputs.rep_elimin_rate
        self.time_of_first_dose = DEInputs.rep_time_1
        self.dosing_interval = DEInputs.rep_dosing_period

    def start(self):
        self.rep_block_string = self.set_rep_blocker_drug(self.met_rate, self.elimin_rate,
                                                          self.time_of_first_dose, self.dosing_interval)

        self.add_free_floating_antimony(model_string=self.rep_block_string, step_size=1,
                                        model_name='viral_rep_blocker_model')
        self.blocker_rr = get_roadrunner_for_single_antimony('viral_rep_blocker_model')
        self.shared_steppable_vars['rep_blocker_active'] = False
        self.base_rate = replicating_rate
        self.shared_steppable_vars['rep_ec50'] = .5
        self.shared_steppable_vars['rep_met'] = self.met_rate
        self.shared_steppable_vars['rep_elim'] = self.elimin_rate
        self.shared_steppable_vars['rep_dosing_interval'] = self.dosing_interval

    def step(self, mcs):

        if self.shared_steppable_vars['changed']:
            self.sbml.viral_rep_blocker_model['met_rate'] = self.shared_steppable_vars['rep_met']
            self.sbml.viral_rep_blocker_model['elimin_rate'] = self.shared_steppable_vars['rep_elim']
            self.sbml.viral_rep_blocker_model['dosing_interval'] = \
                self.shared_steppable_vars['rep_dosing_interval']
        self.blocker_rr.timestep()
        self.update_rmax()

    def calculate_rmax(self, drug_amount):
        rate_ec50 = self.shared_steppable_vars['rep_ec50']
        return (1 - nCoVUtils.hill_equation(drug_amount, self.shared_steppable_vars['rep_ec50'], 1)) * self.base_rate

    def update_rmax(self):
        self.shared_steppable_vars['r_max'] = self.calculate_rmax(self.sbml.viral_rep_blocker_model['drug'])
        for cell in self.cell_list_by_type(self.UNINFECTED, self.VIRUSRELEASING, self.INFECTED):
            if self.shared_steppable_vars['rep_blocker_active']:
                cell.sbml.viralReplication['replicating_rate'] = self.shared_steppable_vars['r_max']
            else:
                cell.sbml.viralReplication['replicating_rate'] = self.base_rate
...
class SimDataSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        self.intern_data_win = self.add_new_plot_window(title='Internalization blocker level',
                                                        x_axis_title='MCS',
                                                        y_axis_title='Concentration (AU)',
                                                        x_scale_type='linear',
                                                        y_scale_type='linear',
                                                        grid=True,
                                                        config_options={'legend': True})
        self.intern_data_win.add_plot("Prodrug", style='Dots', color='blue', size=5)
        self.intern_data_win.add_plot("Active_Metabolite", style='Dots', color='red', size=5)

        self.rep_data_win = self.add_new_plot_window(title='Replication blocker level',
                                                     x_axis_title='MCS',
                                                     y_axis_title='Concentration (AU)',
                                                     x_scale_type='linear',
                                                     y_scale_type='linear',
                                                     grid=True,
                                                     config_options={'legend': True})
        self.rep_data_win.add_plot("Prodrug", style='Dots', color='blue', size=5)
        self.rep_data_win.add_plot("Active_Metabolite", style='Dots', color='red', size=5)

        self.red_intern_rate = self.add_new_plot_window(title='Internalization affinity % of maximum',
                                                        x_axis_title='MCS',
                                                        y_axis_title='%',
                                                        x_scale_type='linear',
                                                        y_scale_type='linear',
                                                        grid=True)
        self.red_intern_rate.add_plot('intern_affinity', style='Dots', color='red', size=5)

        self.rep_rate = self.add_new_plot_window(title='Viral replication rate % of maximum',
                                                 x_axis_title='MCS',
                                                 y_axis_title='%',
                                                 x_scale_type='linear',
                                                 y_scale_type='linear',
                                                 grid=True)
        self.rep_rate.add_plot('rep_rate', style='Dots', color='red', size=5)
    ...
    def step(self, mcs):
        ...
        self.intern_data_win.add_data_point('Prodrug', mcs, self.sbml.viral_internal_blocker_model['prodrug'])
        self.intern_data_win.add_data_point('Active_Metabolite', mcs, self.sbml.viral_internal_blocker_model['drug'])

        self.rep_data_win.add_data_point('Prodrug', mcs, self.sbml.viral_rep_blocker_model['prodrug'])
        self.rep_data_win.add_data_point('Active_Metabolite', mcs, self.sbml.viral_rep_blocker_model['drug'])

        self.red_intern_rate.add_data_point('intern_affinity', mcs, 100 * self.shared_steppable_vars['k_on'] / kon)

        self.rep_rate.add_data_point('rep_rate', mcs, 100 * self.shared_steppable_vars['r_max'] / replicating_rate)
...
class SlidersSteppable(SteppableBasePy):
    ...
    def add_steering_panel(self):
        ...
        self.add_steering_param(name='Activate internalization blocker treatment', val=False, enum=[False, True],
                                widget_name='combobox')
        self.add_steering_param(name='Internalization blocker EC50', val=.5, min_val=0.0001, max_val=5,
                                widget_name='slider')
        self.add_steering_param(name='Internalization blocker metabolization rate (1/MCS)',
                                val=self.shared_steppable_vars['intern_met'],
                                min_val=0.0001, max_val=100, widget_name='slider')
        self.add_steering_param(name='Internalization blocker elimination rate (1/MCS)',
                                val=self.shared_steppable_vars['intern_elim'],
                                min_val=0.0001, max_val=100, widget_name='slider')
        self.add_steering_param(name='Internalization blocker dosing period (MCS)',
                                val=self.shared_steppable_vars['intern_dosing_interval'],
                                min_val=1, max_val=100, widget_name='slider')

        self.add_steering_param(name='Activate replication blocker treatment', val=False, enum=[False, True],
                                widget_name='combobox')
        self.add_steering_param(name='Replication blocker EC50', val=.5, min_val=0.0001, max_val=5,
                                widget_name='slider')
        self.add_steering_param(name='Replication blocker metabolization rate (1/MCS)',
                                val=self.shared_steppable_vars['rep_met'],
                                min_val=0.0001, max_val=100, widget_name='slider')
        self.add_steering_param(name='Replication blocker elimination rate (1/MCS)',
                                val=self.shared_steppable_vars['rep_elim'],
                                min_val=0.0001, max_val=100, widget_name='slider')
        self.add_steering_param(name='Replication blocker dosing period (MCS)',
                                val=self.shared_steppable_vars['rep_dosing_interval'],
                                min_val=1, max_val=100, widget_name='slider')
    ...
    def process_steering_panel_data(self):
        ...
        self.shared_steppable_vars['intern_blocker_active'] = self.get_steering_param('Activate internalization '
                                                                                      'blocker treatment')
        self.shared_steppable_vars['intern_ec50'] = self.get_steering_param('Internalization blocker EC50')
        self.shared_steppable_vars['intern_met'] = self.get_steering_param('Internalization blocker metabolization '
                                                                           'rate (1/MCS)')
        self.shared_steppable_vars['intern_elim'] = self.get_steering_param('Internalization blocker metabolization '
                                                                            'rate (1/MCS)')
        interval = self.get_steering_param('Internalization blocker dosing period (MCS)')
        self.shared_steppable_vars['intern_dosing_interval'] = int(interval)

        self.shared_steppable_vars['rep_blocker_active'] = self.get_steering_param('Activate replication '
                                                                                   'blocker treatment')
        self.shared_steppable_vars['rep_ec50'] = self.get_steering_param('Replication blocker EC50')
        self.shared_steppable_vars['rep_met'] = self.get_steering_param('Replication blocker metabolization '
                                                                        'rate (1/MCS)')
        self.shared_steppable_vars['rep_elim'] = self.get_steering_param('Replication blocker metabolization '
                                                                         'rate (1/MCS)')
        interval = self.get_steering_param('Replication blocker dosing period (MCS)')
        self.shared_steppable_vars['rep_dosing_interval'] = int(interval)

        self.shared_steppable_vars['changed'] = self.steering_param_dirty()
